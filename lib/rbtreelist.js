...
Node.prototype.setSibling = function (dir, node, splice) {
    ...
    // if (this.data === 6542 || (node && node.data === 6542)) {debugger;}
...
RBTreeList.prototype._gapAndSize = function (setIndex, insertedNode, splice) {
    ...
    if (setIndex > lastNodeIndex) {
        ...
        var node = insertedNode.next;
        var nodeIndex = this.indexOfNode(node);
        var gapStart = nodeIndex - node.leftGapCount;
...
RBTreeList.prototype.set = function (setIndex, data, splice) {
    ...
    // If the length of the list is increasing by more than one as
    // a result of this `set` a hole/gap will be created and we
    // should handle that here.
    // if (setIndex >= this.length + 1) {
...
RBTreeList.prototype.unset = function(unsetIndex, remove) {
    ...
    while (node.getChild(dir) !== null) {
        ...
        var cmp = this._comparator(unsetIndex, index);
    ...
    if (found !== null) {
        ...
        // Fix the broken link chain and redistribute gap
        replacement.setSibling(true, found.next);
        if (found.next) {
            ...
            found.next.setSibling(false, replacement);
        }
